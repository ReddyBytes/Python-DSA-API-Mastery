...
> "Ey, one server handling all the traffic is like one auto-rickshaw trying to carry the whole of Hyderabad during Ganesh Chaturthi," says **Ganesh**, a Telugu SRE who has spent years distributing traffic across server fleets. "You need a traffic police at every junction — that is your load balancer, directing vehicles so no single road jams up."
...
- [1. The Problem: One Restaurant, Five Hundred Hungry People](#1-the-problem)
- [2. What a Load Balancer Actually Does](#2-what-a-load-balancer-does)
- [3. Load Balancing Algorithms](#3-algorithms)
  - [Round Robin](#round-robin)
  - [Weighted Round Robin](#weighted-round-robin)
  - [Least Connections](#least-connections)
  - [IP Hash](#ip-hash)
  - [Random](#random)
- [4. L4 vs L7 Load Balancing](#4-l4-vs-l7)
  - [L4 Load Balancing](#l4-load-balancing)
  - [L7 Load Balancing](#l7-load-balancing)
  - [Choosing: L4 or L7?](#choosing-l4-or-l7)
- [5. Sticky Sessions](#5-sticky-sessions)
  - [The Problem Story](#sticky-problem-story)
  - [How Sticky Sessions Work](#how-sticky-sessions-work)
  - [The Real Problem with Sticky Sessions](#real-problem-sticky)
- [6. Health Checks](#6-health-checks)
  - [Active Health Checks](#active-health-checks)
  - [Passive Health Checks](#passive-health-checks)
  - [Graceful Shutdown](#graceful-shutdown)
- [7. High Availability for the Load Balancer Itself](#7-high-availability)
  - [Active-Passive Load Balancer Pair](#active-passive-pair)
  - [DNS Failover](#dns-failover)
  - [Anycast](#anycast)
- [8. Real-World Load Balancers](#8-real-world-lbs)
- [9. The Mental Models to Carry Forward](#9-mental-models)
- [10. Mini Exercises](#10-mini-exercises)
- [11. Learning Priority](#11-learning-priority)
- [Fire Summary](#fire-summary)

<a id="1-the-problem"></a>

# 1. The Problem: One Restaurant, Five Hundred Hungry People

"Imagine you open a biryani restaurant in Ameerpet," Ganesh begins, leaning forward. "You hire one waiter — call him Server 1."
...
"In Hyderabad traffic terms," Ganesh says, "Option A is widening one road to 50 lanes — impossible. Option B is building 10 parallel roads with traffic signals coordinating flow."

[Back to Top](#top)

<a id="2-what-a-load-balancer-does"></a>
...
"Think of it like a reception desk at a government office," Ganesh explains. "Citizens see one building, one entrance. But inside, the receptionist sends you to counter 3 or counter 7 based on who is free. The citizen does not need to know the internal layout."

[Back to Top](#top)

<a id="3-algorithms"></a>

# 3. Load Balancing Algorithms — How to Decide Who Gets the Next Request
...
# 4. L4 vs L7 Load Balancing — The Two Layers

"This is where it gets interesting," Ganesh says. "Imagine two kinds of traffic police. One only looks at the number plate — where is this vehicle registered? That is L4. The other opens the boot, checks the cargo, reads the driver's ID — that is L7. Both useful, different costs."
...
<a id="5-sticky-sessions"></a>

# 5. Sticky Sessions — When You Need the Same Client to Land on the Same Server
...
<a id="sticky-problem-story"></a>

## The Problem Story

"I once debugged this exact issue," Ganesh recalls. "Users in Vizag were losing their shopping carts every other click. The developer had stored session data in process memory — a Python dict sitting on one server."
...
<a id="how-sticky-sessions-work"></a>
...
"The fix is not stickier glue," Ganesh summarizes. "The fix is removing the thing that needs gluing. Externalize state to Redis, and every server becomes interchangeable — like any auto-rickshaw can pick up any passenger."

[Back to Top](#top)

<a id="6-health-checks"></a>

# 6. Health Checks — How the Load Balancer Knows a Server Is Down
...
"A load balancer without health checks is like a traffic signal that never turns red," Ganesh says. "It will happily send cars into a collapsed bridge."
...
<a id="7-high-availability"></a>

# 7. High Availability for the Load Balancer Itself
...
"The irony is not lost on us SREs," Ganesh grins. "You add redundancy and create a new bottleneck. It is turtles all the way down — but you only need to go two turtles deep."

<a id="active-passive-pair"></a>
...
| Concept | One-Liner |
|---------|-----------|
| Load Balancer | A router + health monitor that distributes traffic across servers |
| Round Robin | Take turns in order — simple, equal distribution |
| Weighted Round Robin | Faster servers get proportionally more traffic |
| Least Connections | Route to whichever server is least busy right now |
| IP Hash | Same client always hits the same server (deterministic) |
| L4 (Transport) | Fast, blind — routes TCP streams without reading content |
| L7 (Application) | Smart — reads HTTP, routes by URL/header/cookie |
| Sticky Sessions | Workaround for stateful servers; fix is to externalize state |
| Health Checks | Active (pings) + passive (observe failures) keep routing accurate |
| HA for LB | Active-passive pair with floating VIP eliminates LB as SPOF |
| Graceful Shutdown | Drain connections before killing a server |
...
| Next | [09 — Message Queues](../09_message_queues/theory.md) |
| Home | [README.md](../README.md) |

**[Back to README](../README.md)**

**Prev:** [Storage & CDN](../07_storage_cdn/theory.md) | **Next:** [Message Queues](../09_message_queues/theory.md)
