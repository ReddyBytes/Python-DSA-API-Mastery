...
# Storage and CDN Theory

> "Every photo you have ever uploaded, every video you have ever streamed, every file attachment in every email — it lives somewhere specific, in a specific format, for very specific reasons." — Anitha, Telugu cloud engineer, explaining storage architecture to her team after a catastrophic S3 misconfiguration took down their media pipeline for six hours.

<a id="toc"></a>

## Table of Contents

- [1. The Setup: Three Problems, Three Solutions](#1-the-setup)
  - [Anitha's Storage Awakening](#anithas-storage-awakening)
- [2. Block Storage — The Raw Hard Drive](#2-block-storage)
  - [Characteristics](#block-characteristics)
- [3. File Storage — The Shared Network Drive](#3-file-storage)
  - [Characteristics](#file-characteristics)
- [4. Object Storage — Files + Metadata via HTTP API](#4-object-storage)
  - [Characteristics](#object-characteristics)
  - [The Comparison at a Glance](#storage-comparison)
- [5. Why Object Storage Won the Web](#5-why-object-storage-won)
  - [Unlimited Objects](#unlimited-objects)
  - [HTTP Access Means No Infrastructure](#http-access)
  - [Built-in Redundancy](#built-in-redundancy)
  - [Price](#price-advantage)
- [6. S3 Concepts You Need to Know Cold](#6-s3-concepts)
  - [Buckets](#buckets)
  - [Keys](#keys)
  - [Presigned URLs](#presigned-urls)
  - [Versioning](#versioning)
  - [Lifecycle Policies](#lifecycle-policies)
- [7. CDN — Making Your Content Fast Everywhere](#7-cdn)
  - [The Problem: Physics](#the-problem-physics)
  - [The Solution: A Network of Servers Near Your Users](#cdn-solution)
  - [Origin Pull vs Pre-pushed Content](#origin-pull-vs-push)
  - [Cache-Control Headers](#cache-control-headers)
  - [Cache Invalidation](#cache-invalidation)
  - [When to Use a CDN vs When Not To](#cdn-use-cases)
- [8. What to Store Where](#8-what-to-store-where)
  - [User Profile Photos](#user-profile-photos)
  - [Database Backups](#database-backups)
  - [Uploaded Documents](#uploaded-documents)
  - [Application Configuration](#application-configuration)
- [9. The Presigned Upload Pattern](#9-presigned-upload-pattern)
  - [The Naive Approach (and Why It Fails)](#naive-approach)
  - [The Presigned URL Pattern (Correct Approach)](#presigned-url-pattern)
- [10. The Mental Models to Keep](#10-mental-models)
- [11. Mini Exercises](#11-mini-exercises)
...
# 1. The Setup: Three Problems, Three Solutions

<a id="anithas-storage-awakening"></a>

## Anitha's Storage Awakening

Anitha had been a cloud engineer for three years when her company's startup accelerator asked her to architect storage for a Dropbox-like product. She sat down at her desk in Hyderabad, opened her laptop, and thought: "Users upload files. I need to store them. I will just write them to disk."

That works on one server. Then you get 10,000 users. You need more servers. Now which server has which file? What if a server crashes? Anitha learned this the hard way when a single EBS volume failure wiped out 200 user uploads that had no redundancy.

"The storage problem is not about storing data," Anitha tells new engineers on her team. "It is about storing data so that it survives failures, scales to millions of users, and does not bankrupt the company. The industry solved it three different ways, each making different trade-offs."

[Back to Top](#top)

<a id="2-block-storage"></a>

# 2. Block Storage — The Raw Hard Drive

Block storage is the closest thing to plugging a hard drive directly into a computer. It gives you raw blocks of storage — no file structure, no metadata. The operating system (or database) decides what to do with those blocks.

"Think of block storage like a raw plot of land," Anitha explains to junior engineers. "You get the raw space, but you have to build everything on top — the filesystem, the directory structure, everything. And only one person can build on that plot at a time."
...
**The mental model:** A hard drive screwed into your server. Fast, low-latency, but only that one server can use it.
...
<a id="3-file-storage"></a>

# 3. File Storage — The Shared Network Drive

File storage is what you get when multiple servers need to share a filesystem. It exposes a directory tree with files and folders, accessible over the network.

"When I joined my first company," Anitha recalls, "we had eight app servers all needing to read the same configuration files. Block storage could not help us — you cannot mount the same EBS volume on eight instances simultaneously. File storage was the answer: one shared drive that all eight servers could mount."
...
**The mental model:** A network drive that multiple computers mount simultaneously. When Server A writes a file, Server B can immediately read it.
...
<a id="4-object-storage"></a>

# 4. Object Storage — Files + Metadata via HTTP API

Object storage is the radical departure from the other two. There is no filesystem. No directories (technically). No mounting. Just a flat namespace of objects, each with a key and metadata, accessed entirely over HTTP.

"Object storage changed everything for us," Anitha says. "Before S3, we ran a fleet of file servers — NFS mounts, backup scripts, capacity planning meetings every quarter. Then one day I replaced all of it with a single S3 bucket and a presigned URL generator. The file servers disappeared. The capacity meetings disappeared. And our storage bill dropped by 70%."
...
**The mental model:** A giant dictionary where keys are file paths and values are files. You access it with HTTP GET/PUT. No mounting. No servers to manage.
...
<a id="5-why-object-storage-won"></a>

# 5. Why Object Storage Won the Web

When the web needed to store billions of profile pictures and petabytes of video, object storage turned out to be the only thing that actually scaled.

Anitha remembers the moment it clicked for her: "I was calculating costs for storing 50 TB of user-uploaded media. EBS would cost $5,000/month. EFS would cost $15,000/month. S3 Standard would cost $1,150/month. And S3 needed zero servers to manage. That is when I understood why every internet company runs on object storage."

Here is why:

<a id="unlimited-objects"></a>

## Unlimited Objects

A filesystem has inodes — there is a maximum number of files you can store, regardless of disk space. Object storage has no such limit. Amazon S3 reportedly stores over 100 trillion objects. You cannot run out of "slots."

<a id="http-access"></a>

## HTTP Access Means No Infrastructure

With block or file storage, you need servers to mount the volumes and serve the files. Object storage is just an HTTP endpoint. Your mobile app can upload directly to S3. Your CDN can pull directly from S3. No file server in the middle.
...
<a id="built-in-redundancy"></a>

## Built-in Redundancy

S3 stores each object across multiple physical locations automatically. The documented durability is 99.999999999% (11 nines). You did not build any of that redundancy — it came with the service.

<a id="price-advantage"></a>

## Price

At $0.023 per GB per month, storing 1 TB on S3 costs about $23/month. Running your own file server with equivalent redundancy would cost far more in hardware, power, operations, and engineering time.
...
<a id="6-s3-concepts"></a>

# 6. S3 Concepts You Need to Know Cold

"In every system design interview I have conducted," Anitha says, "candidates who understand S3 deeply — buckets, keys, presigned URLs, versioning, lifecycle policies — they stand out immediately. These are not obscure features. They are the building blocks of every production storage system on AWS."
...
The key is the full "path" to an object. S3 is actually flat — there are no real directories. The slashes in keys are just characters, but the console displays them as folder hierarchy.
...
This is one of the most important patterns. A presigned URL is a time-limited, cryptographically signed URL that grants temporary access to an object without requiring the requester to have AWS credentials.
...
Why this matters: your app server does not become a file transfer bottleneck. 1 GB video uploads bypass your server entirely.

<a id="versioning"></a>

## Versioning

With versioning enabled, S3 keeps every version of every object instead of overwriting.
...
<a id="lifecycle-policies"></a>

## Lifecycle Policies

Automatically transition or delete objects based on age. This is how you manage storage costs at scale.

"I set up a lifecycle policy on our backup bucket," Anitha explains, "and it saved us $4,000/month without any code changes. Data just flows downhill to cheaper tiers automatically."
...
# 7. CDN — Making Your Content Fast Everywhere

"The first time I deployed a CDN," Anitha recalls, "our page load times in Australia dropped from 3.2 seconds to 400 milliseconds. Nothing else changed — same servers, same code, same images. We just moved the data closer to the users. That is the entire trick."

<a id="the-problem-physics"></a>

## The Problem: Physics

Your S3 bucket lives in us-east-1 (Northern Virginia). A user in Sydney, Australia requests a 5 MB image. That request travels:
...
A CDN (Content Delivery Network) is exactly that: dozens or hundreds of servers (called Points of Presence, or PoPs) distributed globally, caching your content close to users.
...
<a id="cache-control-headers"></a>

## Cache-Control Headers: Teaching the CDN When to Cache

When your origin server (or S3) sends a response, it includes headers that tell the CDN (and browsers) how long to cache the content.
...
The key insight: static assets get long TTLs by putting a content hash in the filename (e.g. `bundle.a3f9c2.js`). If the file changes, the filename changes, so the CDN treats it as a brand new object.
...
<a id="8-what-to-store-where"></a>

# 8. What to Store Where

This is what gets asked in design interviews. Anitha trains her team's intuition with these patterns:

"Every storage decision in system design comes down to three questions: How often is it read? How often is it written? Who needs access? Answer those and the storage type picks itself."
...
<a id="9-presigned-upload-pattern"></a>

# 9. The Presigned Upload Pattern

This is the pattern you want to know for design interviews. It comes up any time you design a system with user-uploaded content.

"I have seen this pattern asked in every single system design interview that involves file uploads," Anitha notes. "Instagram, Dropbox, Google Drive, Slack file sharing — they all use some variation of presigned URLs. Master this one pattern and you can design the upload flow for any system."
...
<a id="10-mental-models"></a>

# 10. The Mental Models to Keep

Anitha keeps these decision trees pinned above her desk. "When a design question comes up in an interview or a sprint planning meeting, I run through these trees in my head. The answer falls out in seconds."
...
<a id="11-mini-exercises"></a>

# 11. Mini Exercises

**1.** You are designing Instagram. Users upload photos. Where do photos live? How do you serve them to followers? Sketch the upload flow and the read flow.

**2.** Your company stores 10 TB of database backups. Currently on EBS snapshots at $0.05/GB. You have not restored a backup in 2 years. How would you reduce this cost using lifecycle policies?

**3.** A user's profile avatar URL is `https://cdn.myapp.com/avatars/user-99.jpg` with a 7-day CDN TTL. The user updates their avatar. How do you make sure people see the new avatar quickly? What is the best strategy?

**4.** Your engineering team suggests: "Just serve user-uploaded PDFs directly from the app server — it is simpler." List three reasons why presigned URLs to S3 are better for production use.
...
|---------|-------------|
| Block Storage | Raw disk for one server — databases and OS volumes |
| File Storage | Shared filesystem for multiple servers — legacy apps and config |
| Object Storage | HTTP-accessible flat namespace — web content, uploads, backups |
| S3 Buckets/Keys | Globally unique namespace + flat key structure (no real directories) |
| Presigned URLs | Time-limited signed URLs for secure direct upload/download |
| Versioning | Keep every version of every object — audit trails and protection |
| Lifecycle Policies | Automatically move aging data to cheaper tiers |
| CDN | Cache static content at edge locations close to users |
| Cache-Control | Headers that tell CDN/browser how long to cache |
| Cache Invalidation | Content-hashed filenames are better than purging |
| Presigned Upload Pattern | Client uploads directly to S3, server only handles metadata |
| Storage Decision Tree | Read/write frequency + access pattern = storage type |

**Anitha's golden rule:** "If you can access it over HTTP, put it in object storage. If you need a filesystem, use block or file storage. If users everywhere need it fast, put a CDN in front. That covers 95% of storage decisions in system design."
...
| | |
|---|---|
| Previous | [06 Caching](../06_caching/theory.md) |
| Next | [08 Load Balancing](../08_load_balancing/theory.md) |
