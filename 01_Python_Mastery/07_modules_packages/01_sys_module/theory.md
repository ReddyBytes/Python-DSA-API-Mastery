# 🖥️ The sys Module — Python's Interpreter Interface

> The `sys` module is your window into the Python interpreter itself — giving you access to the command-line arguments, module cache, standard streams, and exit controls that live inside the running process.
> Think of it as cockpit instruments: `os` talks to the outside world; `sys` shows what's happening inside the engine right now.
...
`sys.argv` · `sys.path` · `sys.exit()` · `sys.stdin/stdout/stderr` · `sys.modules`

**Should Learn** — Comes up regularly:
`sys.version_info` · `sys.platform` · `sys.executable` · `sys.getrecursionlimit()`

**Good to Know** — Specific situations:
`sys.getsizeof()` · `sys.exc_info()` · `sys.intern()`

**Reference** — Know it exists:
`sys.flags` · `sys.implementation` · `sys.byteorder`
...
| ➡️ Next Subfolder | [02_argparse →](../02_argparse/theory.md) |

---

**Related:** [02_argparse](../02_argparse/theory.md) · [03_subprocess](../03_subprocess/theory.md) · [04_virtual_environments](../04_virtual_environments/theory.md)
