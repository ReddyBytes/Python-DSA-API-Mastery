...
> Dates look simple until you have to compare a "2024-03-15" string from a CSV, a Unix timestamp from an API, and a timezone-aware datetime from a database — and get them all into the same unit.
> The `datetime` module is Python's toolbox for creating, parsing, formatting, and doing arithmetic on points in time.

---

## 📌 Learning Priority

**Must Learn** — Every Python project involving time:
`datetime.now()` · `datetime.fromisoformat()` · `strftime()` · `strptime()` · `timedelta` · `.timestamp()`

**Should Learn** — Production necessity:
`timezone.utc` · `pytz` / `zoneinfo` · timezone-aware vs naive · `datetime.utcnow()` pitfall

**Good to Know** — Common patterns:
`date.today()` · `datetime.combine()` · `.replace()` · `calendar` module basics

**Reference** — Specialized use:
`time.perf_counter()` · `time.sleep()` · `dateutil` library
...
Think of the `datetime` module like a set of measuring instruments: one measures just the day on the calendar, one measures just the position of clock hands, one measures both together, and one measures the gap between any two readings.
...
A datetime object is just a structured snapshot of a moment. You can capture it from right now, specify it exactly, or reconstruct it from a number of seconds elapsed since a fixed reference point.
...
If a `datetime` is a point on a timeline, a **`timedelta`** is the ruler you use to measure between two points, or to jump forward and backward from one. You can add days, subtract hours, and express any duration as a single number of seconds.
...
Every system that receives a timestamp from your code needs it as a string in a particular shape. `strftime` is the mold: you describe the exact shape using percent-codes, and it stamps the datetime into that form.
...
If `strftime` is a mold that stamps a datetime into a string shape, then `strptime` is the opposite: it reads a string of a known shape and pours it back into a datetime object. You are essentially telling Python: "I know this string looks like this — find the date inside it."
...
Timezones are where most datetime bugs live in production. Imagine a bank that records every transaction as "3:00 PM" with no timezone. When the clocks change for daylight saving time, did that 3:00 PM transaction happen before or after the adjustment? The bank cannot know — and neither can Python if you leave timezone information off your datetimes.
...
Think of a Unix timestamp as a universal language for time: no matter where in the world a server is running, "1705334400" means exactly one moment — January 15, 2024 at 16:00:00 UTC. There are no timezones to argue about and no ambiguity. It is just a count of seconds from a fixed point that everyone agreed on.
...
Where `datetime` is a calendar on the wall, the `time` module is a stopwatch in your hand. It cares less about what day it is and more about how many seconds just elapsed between two lines of code — or whether to pause execution for a moment.
...
Real codebases deal with time constantly: tokens expire, queries filter by date range, logs need timestamps, and age calculations must survive DST transitions. These patterns cover the most common scenarios.
