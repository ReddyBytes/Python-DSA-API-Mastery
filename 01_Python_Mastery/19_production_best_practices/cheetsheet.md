...
poetry new my_project              # scaffold with structure
poetry install                     # install all deps, create .venv
poetry install --without dev       # production (skip dev deps)

# Dependencies
poetry add httpx                   # add runtime dep
poetry add --group dev pytest      # add dev-only dep
poetry remove httpx                # remove dep
poetry update httpx                # update one dep
poetry lock --check                # verify lock file is current

# Running
poetry run pytest                  # run in managed venv
poetry shell                       # spawn venv shell
...
|---|---|---|
| Generated by | `poetry install` | `pip freeze` or `pip-compile` |
| Includes transitive deps | Yes | Yes (pip freeze) |
| Tracks which are direct vs transitive | Yes | No |
| Used with | poetry | pip |
...
# Commit this instead (no real values)
.env.example

# Load in code
from dotenv import load_dotenv; load_dotenv()

# In production: inject via platform, not .env files
# K8s: envFrom: secretRef / AWS: Parameter Store / ECS: secrets
```
...
|---|---|---|
| Speed | ~1–5s | ~0.05s (10–100x faster) |
| Tool count | 2+ (+ plugins) | 1 |
| Auto-fix | Limited | Yes (`--fix`) |
| Config | Multiple files | pyproject.toml only |
| Rules | pycodestyle + pyflakes | All of the above + bugbear + pyupgrade + more |

```bash
# ruff replaces both:
ruff check src/             # lint
ruff check --fix src/       # auto-fix
# isort is built in — ruff I rules handle import sorting
```
...
**Related Topics:** [Theory](./theory.md) · [Coding Standards](./01_coding_standards/theory.md) · [Project Structure & Packaging](./02_project_structure_packaging/theory.md) · [Environment Management](./03_environment_management/theory.md)
